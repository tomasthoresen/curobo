...
inline bool configure_shared_memory(KernelType kernel, LBFGSLaunchCfg& config) {
    ...
    // hipcc does not implicitly convert a function pointer to const void*
    // (nvcc does); cast explicitly. Valid on both platforms.
    cudaError_t result = cudaFuncSetAttribute(
        reinterpret_cast<const void*>(kernel), cudaFuncAttributeMaxDynamicSharedMemorySize, config.max_shared_memory);
                ...
                const int v_dim, const bool stable_mode, const bool use_shared_buffers)
{
    ...
    if (config.use_shared_buffers && config.shared_memory_size <= config.max_shared_memory) {
        // Launch through a local function pointer: the hipify pass mistranslates
        // a member-access launch (obj.member<<<...>>>) but handles a plain name.
        auto shared_memory_kernel = kernel_pair.shared_memory_kernel;
        shared_memory_kernel<<<config.blocksPerGrid, config.threadsPerBlock, config.shared_memory_size, stream>>>(
        ...
        auto stable_kernel = kernel_pair.stable_kernel;
        stable_kernel<<<config.blocksPerGrid, config.threadsPerBlock, basic_smem_size, stream>>>(
            step_vec.data_ptr<float>(),
            rho_buffer.data_ptr<float>(),
            y_buffer.data_ptr<float>(),
            s_buffer.data_ptr<float>(),
