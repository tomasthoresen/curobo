...
__forceinline__ __device__ ValueType warp_reduce(ValueType v, const int elems)
{
    ...
    for (uint8_t offset = 32 / 2; offset > 0; offset /= 2) {
        ...
        // 1ULL avoids shifting a 32-bit int by >= 32 (undefined) when
        // lane_idx + offset exceeds 31; those high bits are never set in the
        // wave32 mask, so the test result is unchanged.
        if ((1ULL << (lane_idx + offset)) & mask) {
